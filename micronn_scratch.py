...
clib = ct.CDLL("./tensor.so")


class CTensor(ct.Structure):
    _fields_ = [
        ("value", ct.POINTER(ct.c_float)),
...
def create_ctensor(value: list) -> CTensor:
    test_value = copy.deepcopy(value)
    tensor_shape = []
    tensor_ndim = 0
    while isinstance(test_value, list):
        if len(test_value) == 0:
            break
        tensor_ndim += 1
    ...
    tensor = CTensor()
    tensor_value = ct.cast(
        (ct.c_float * len(flattened_value))(*flattened_value),
        ct.POINTER(ct.c_float),
    )
    tensor_shape = ct.cast(
...
def get_ctensor_element(tensor: CTensor, idx: tuple):
    return clib.ctensor_element(
        ct.byref(tensor), (ct.c_size_t * len(idx))(*idx)
    )[0]
...
class Tensor:
    ...
    def __iter__(self):
        for i in range(self._ctensor.od_size):
